...
class XMLParagraph(fontID: String, position: XMLPosition, options: XMLParagraphOptionsContainer, lines: List[XMLLine], linesSeparator: String, text: String, enumFormat: String) {
	...
	def addLine(line: XMLLine): XMLParagraph = new XMLParagraph(fontID, new XMLPosition(scala.math.min(position.getX, line.getPosition.getX), position.getY, scala.math.max(position.getX + position.getWidth, line.getPosition.getX + line.getPosition.getWidth) - scala.math.min(position.getX, line.getPosition.getX), (line.getPosition.getY + line.getPosition.getHeight) - position.getY), options, line :: lines, linesSeparator, text + linesSeparator + line.getText, enumFormat)
